...
function StepCounter() {
  ...
  // On native, this is just a placeholder — the real value comes from
  // StepSensor.getSteps() (SharedPreferences) which fires immediately.
  const cachedSteps =
  ...
  useEffect(() => {
    // Always read from native SharedPreferences as the single source of truth.
    // The BackgroundStepService persists steps even when app is killed,
    // so this will never "reset to 0" on reopen.
    const fetchNativeSteps = async () => {
      if (!isNative || !StepSensor) return; // No-op in browser
      try {
        const result = await StepSensor.getSteps();
        if (result && result.value !== undefined) {
          const nativeSteps = result.value;
          setSteps(nativeSteps);
          // Sync back to localStorage so the rest of the app sees it
          localStorage.setItem("steps", String(nativeSteps));
          localStorage.setItem("steps_date", today);
          localStorage.setItem("kms", (nativeSteps * 0.000762).toFixed(2));
          localStorage.setItem("stepCalories", String(Math.round(nativeSteps * 0.04)));
        }
      } catch (err) {
    ...
    // 1. Immediately fetch on mount
    fetchNativeSteps();

    // 2. Poll every 5 seconds for background updates
    const interval = setInterval(fetchNativeSteps, 5000);

    // 3. Refresh immediately when app comes back to foreground
